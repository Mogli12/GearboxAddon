...
mrGearboxMogliGlobals.combineDynamicRatio   = 0.8
...
-- state variables with setter methods	
	...
	mrGearboxMogli.registerState( self, "TargetSpeed",   -1 )
...
--**************************************************************************************************		
    ...
    local sqm   = factor * speed * width / 3.6
		...
		local dynamicRatio = Utils.getNoNil( getXMLFloat(xmlFile, xmlString .. ".combine#dynamicRatio"), self.mrGbMG.combineDynamicRatio )
		...
		if math.abs( self.mrGbMG.combineDefaultSpeed - speed ) > 0.1 then
			dp0 = dp0 * self.mrGbMG.combineDefaultSpeed / speed 
			dc0 = dc0 * self.mrGbMG.combineDefaultSpeed / speed 
		end
		...
		self.mrGbMS.ThreshingMinRpm              = Utils.getNoNil( getXMLFloat(xmlFile, xmlString .. ".combine#minRpm")                      , 0.5 * self.mrGbMS.IdleRpm + 0.5 * self.mrGbMS.RatedRpm )
	...
	while true do
		...
		if ratio==nil then
			...
			if s ~= nil and self.mrGbMS.Gears[g] ~= nil then
				ratio = s / (3.6*self.mrGbMS.Gears[g].gear)
			end
		end 
		if ratio==nil then
			break 
...
--**********************************************************************************************************			
	if mrGearboxMogli.mbIsActiveForInput( self, false ) then					
		...
		if      not ( self.steeringEnabled ) 
				or  self.cruiseControl.state > 0 
				or  self.tempomatMogliV14.keepSpeedLimit ~= nil then
			self:mrGbMSetState( "TargetSpeed", -1 ) 
		elseif  self.mrGbMS.ConstantRpm 
				and ( self.mrGbMS.Hydrostatic or self:mrGbMGetAutomatic() ) then
			local t = self.mrGbMS.TargetSpeed
			local m
			if self.mrGbMS.ReverseActive then
				m = self.motor.maxBackwardSpeed 
			else
				m = self.motor.maxForwardSpeed 
			end
			m = m * 3.6 
			if not self:mrGbMGetAutomatic() then
				m = math.min( m, self.mrGbMD.Speed )
			end
			local n = 0
			if not ( self.mrGbMS.Hydrostatic ) then
				n = self.mrGbMD.Speed * self.mrGbMS.IdleRpm / self.mrGbMS.RatedRpm 
			end
			
			if m > 0 then
				if     self.mrGbMS.TargetSpeed < 0 then
					if self.mrGbMS.Hydrostatic then
						t = math.max( n, self.lastSpeedReal*3600 )
					else
						t = self.mrGbMD.Speed
					end
				elseif self.axisForwardIsAnalog then
					if self.axisForward > 0 then
						t = self.lastSpeedReal*3600
					else
						t = m * math.max( 0, -self.axisForward )
					end
				elseif self.axisForward > 0 then
					t = t - self.axisForward * 0.010 * dt
				elseif self.axisForward < 0 then
					t = t - self.axisForward * 0.005 * dt
				end
				self:mrGbMSetState( "TargetSpeed", Utils.clamp( t, 0, m ) )
			else
				self:mrGbMSetState( "TargetSpeed", -1 ) 
			end
		else
	...
	if      self.isServer 
			 and self.mrGbMS.OnlyHandThrottle
			and ( driveControlHandBrake or not ( self.isMotorStarted ) ) then
		...
		if self.mrGbMS.IsOn and self.mrGbML.motor ~= nil and self.motor == self.mrGbML.motor then	
          ...
          self.mrGbMD.Load   = 0
					...
					if self.isMotorStarted then
						...
						if self.motor.hydroEff ~= nil and not ( self.motor.noTransmission ) then
					...
					if     self.mrGbMD.lastClutch ~= self.mrGbMD.Clutch
							...
							or ( self.mrGbMS.sendTargetRpm and self.mrGbMD.lastRpm    ~= self.mrGbMD.Rpm   )
...
--if self.mrGbML.motor == nil then return end
	...
	if self.mrGbMS.IsOn and self.mrGbMS.showHud and self.mrGbMS.HudMode > 0 then
		...
		if self.mrGbMS.HudMode == 1 then
			...
			elseif self.mrGbMS.Hydrostatic and self.mrGbMS.ConstantRpm then
			...
			elseif self.mrGbMS.OnlyHandThrottle or self.mrGbMS.HandThrottle > 0 then
				...
				for row,info in pairs( infos ) do
					...
					elseif info == "target1" then
						if col == 1 then
							...
							local sp = self.mrGbMD.Speed * ( ir + self.mrGbMS.HandThrottle * ( 1 - ir ) )
							renderText(ovRight,drawY, deltaY, string.format("%3.1f km/h", sp ))
						end
					elseif info == "target2" then
						if col == 1 then
							renderText(ovLeft, drawY, deltaY, "Target speed")	
						else
							local s = self.mrGbMS.TargetSpeed * self:mrGbMGetTargetRPM() / self.mrGbMS.RatedRpm
							renderText(ovRight, drawY, deltaY, string.format("%3.1f km/h", s ))       
						end
					...
					elseif info == "rpm" then
						if col == 1 then
							...
							renderText(ovRight, drawY, deltaY, string.format("%4.0f rpm", math.floor( rpm * 0.1 +0.5)*10)) 		
						end
...
function mrGearboxMogli:mrGbMOnSetNeutral( old, new, noEventSend )		
	...
	if self.isServer then
		...
		if not ( new ) then
			self:mrGbMSetState( "AutoHold", false )
			if      self.mrGbMS.OnlyHandThrottle 
...
function mrGearboxMogli:newUpdateWheelsPhysics( superFunc, dt, currentSpeed, acc, doHandbrake, requiredDriveMode, ... )
	...
	-- driveControl and GPS
		...
		elseif  self.mrGbMS.TargetSpeed >= 0 
				and not self.mrGbMS.NeutralActive then			
			if self.mrGbMS.TargetSpeed == 0 then
				if self.mrGbMS.Hydrostatic and self.mrGbMS.HydrostaticMin < mrGearboxMogli.eps then
					acceleration = acc
				else
					acceleration = -1
				end
	...
	if     self.mrGbML.gearShiftingNeeded ~= 0
	    or self.motor.noTransmission then
		-- disable motor brake 
		...
		if     ( self.mrGbML.gearShiftingNeeded > 0 and self.mrGbML.gearShiftingNeeded ~= 2 )
				or self.mrGbML.gearShiftingNeeded < 0 and accelerate < 0.001 then
			motorBrakeOn = true
		end
	...
	--if self.mrGbML.debugTimer ~= nil and g_currentMission.time < self.mrGbML.debugTimer and not ( self.mrGbMS.Hydrostatic ) then
		if      mrGearboxMogli.debugGearShift
		  ...
		  if not ( mrGearboxMogli.debugGearShiftHeader ) then
			...
			print(string.format("%6.2f%%: %4.0f Nm, %4.0f Nm, %4.0f Nm, %4.0f U/min, %4.0f U/min, %4.0f U/min, %4.0f U/min, %4.0f U/min, %4.2f %4.2f km/h, %3.1f, %3.1f, %3.1f, %3.0f%%, %d",
...
function mrGearboxMogliMotor:new( vehicle, motor )
	...
	self.lastMotorRpm           = motor.nonClampedMotorRpm
...
function mrGearboxMogliMotor:updateSpeedLimit( dt )
	...
	if self.vehicle.mrGbMS.TargetSpeed >= 0 then
		speedLimit = math.min( speedLimit, self.vehicle.mrGbMS.TargetSpeed * mrGearboxMogli.kmhTOms * self.targetRpm / self.ratedRpm ) 
	end
...
function mrGearboxMogliMotor:getCurMaxRpm( forGetTorque )
	...
	if not ( self.noTransmission ) then		
		...
		if     self.clutchPercent < mrGearboxMogli.eps then
			curMaxRpm = mrGearboxMogli.huge
		elseif self.clutchPercent < 1 then
		--local maxWheelRpm = self.wheelRpm + 0.00981 * self.tickDt * mrGearboxMogli.factor30pi * self:getMogliGearRatio()
		--local maxWheelRpm = ( self.vehicle.lastSpeedReal*self.vehicle.movingDirection*1000 ) * mrGearboxMogli.factor30pi * self:getMogliGearRatio()
...
--local acc             = math.max( math.abs( acceleration ), self.minThrottle )
	...
	self.lastMotorTorque  = self.lastMotorTorque + self.vehicle.mrGbML.smoothFast * ( torque - self.lastMotorTorque )
	...
	if pt > 0 then
		if not ( self.noTransmission 
					or self.noTorque 
					or self.hydroEff ~= nil
					or ( self.vehicle.mrGbMS.TorqueConverter and self.vehicle.mrGbMS.OpenRpm > self.maxPowerRpm - 1 ) ) then
			local mt = self.currentTorqueCurve:get( Utils.clamp( self.lastMotorRpm, self.idleRpm, self.ratedRpm ) ) 
			if mt < pt then
	...
	if self.noTorque or torque <= 0 or acc <= 0 then
		...
		if     self.vehicle.lastAcceleration <= -0.001 then
			acc = 0
		elseif ( self.ptoOn and ( self.hydroEff ~= nil or self.vehicle.mrGbMS.PowerManagement ) )
				or self.vehicle.mrGbMS.HandThrottle > 0.01 then
			if self.nonClampedMotorRpm < self.minRequiredRpm then
				acc = 1
			end
		elseif self.vehicle.mrGbMS.OnlyHandThrottle or self.vehicle.mrGbMS.HandThrottle > 0.01 then
			acc = self.vehicle.mrGbMS.HandThrottle
		elseif self.vehicle.mrGbMS.PowerManagement then
...
function mrGearboxMogliMotor:updateMotorRpm( dt )
	...
	self.lastMotorRpm    = self:getMotorRpm()
	
	if self.lastMotorRpm > self.maxPossibleRpm then
		self.lastMotorRpm = self.maxPossibleRpm
	end
	if mrGearboxMogliMotor.increaseHyroLaunchRpm( self ) then
		self.lastMotorRpm = math.max( self.lastMotorRpm, self.minRequiredRpm * mrGearboxMogli.rpmReduction )	
	end
	
	self.deltaMotorRpm = math.floor( self.lastMotorRpm - self.nonClampedMotorRpm + 0.5 )
	...
	self.lastMotorRpmS = self.lastMotorRpmS + self.vehicle.mrGbML.smoothFast * ( self.lastMotorRpm - self.lastMotorRpmS )
...
--**********************************************************************************************************	
	...
	if     self.vehicle.mrGbMS.OnlyHandThrottle
			or self.vehicle.mrGbMS.HandThrottle > 0.01 then
...
--**********************************************************************************************************		
		...
		if lastNoTransmission then
			self.minThrottle  = math.max( 0.3, handThrottle )
			self.minThrottleS = math.max( 0.3, handThrottle )
	...
	-- hydrostatic drive
			...
			--end
				...
				local d = mrGearboxMogli.hydroEffDiff + ( 1 - math.max( 0, accelerationPedal ) ) * mrGearboxMogli.hydroEffDiffInc
				...
				if hMin0 < mrGearboxMogli.eps then
					hMin0 = mrGearboxMogli.eps
				end
				...
				while true do
					...
					if self.vehicle.cruiseControl.state > 0 then
						hMax = math.min( currentSpeedLimit * mrGearboxMogli.factor30pi * r / minRpmReduced, hMax )
					end
					...
					if isValidEntry or not self.vehicle:mrGbMGetAutomatic() then
						...
						local hTgt  = Utils.clamp( w / t, hMin, hMax )
				
					--if     not accHydrostaticTarget then
					--	self.hydrostaticTarget = nil
					--elseif hMax - hMin < mrGearboxMogli.eps then
					--	self.hydrostaticTarget = nil
					--elseif lastNoTransmission or clutchMode > 1 or g ~= currentGear then
					--	self.hydrostaticTarget = nil
					--elseif self.vehicle.cruiseControl.state > 0 
					--    or not self.vehicle.steeringEnabled then								
					--	local hTarget = Utils.clamp( currentSpeedLimit * mrGearboxMogli.factor30pi * r / t, hMin, hMax )
					--	hTarget = math.min( hTgt, hTarget )
					--	self.hydrostaticTarget = ( hTarget - hMin ) / ( hMax - hMin )							
					--elseif self.vehicle.axisForwardIsAnalog then
					--	self.hydrostaticTarget = math.max( Utils.clamp( ( hMin1 - hMin ) / ( hMax - hMin ), 0, 1 ), accelerationPedal )
					--elseif self.hydrostaticTarget == nil then
					--	self.hydrostaticTarget = Utils.clamp( ( hTgt - hMin ) / ( hMax - hMin ), 0, 1 )
					--elseif accelerationPedal <= -mrGearboxMogli.accDeadZone then											
					--	-- 2.50 seconds 
					--	self.hydrostaticTarget = math.max( 0, self.hydrostaticTarget + accelerationPedal * 0.0004 * self.tickDt )
					--elseif accelerationPedal >=  mrGearboxMogli.accDeadZone then						
					--	-- 3.33 seconds 
					--	local h1 = self.hydrostaticTarget + accelerationPedal * 0.0003 * self.tickDt
					--	local h2 = math.max( ( hMin1 - hMin ) / ( hMax - hMin ), self.hydrostaticTarget )
					--	self.hydrostaticTarget = Utils.clamp( math.min( h1, h2 ) , 0, 1 )							
					--end
						if self.vehicle.mrGbMS.TargetSpeed >= 0 and hMax - hMin > mrGearboxMogli.eps and t > mrGearboxMogli.eps then
							local h = self.vehicle.mrGbMS.TargetSpeed / ( 3.6 * spd )
							self.hydrostaticTarget = Utils.clamp( ( h - hMin ) / ( hMax - hMin ) , 0, 1 )							
						else
							self.hydrostaticTarget = nil
						end
						...
						if     hMin >= hMax then
							h  = hMin
						elseif self.hydrostaticTarget ~= nil then
							h = math.min( hMin + self.hydrostaticTarget * ( hMax - hMin ), hMax1 )
						elseif c > m then
							h = hMin1
						elseif c < n then
							h = hMax1
						...
						if self.vehicle:mrGbMGetAutomatic() then
							...
							if self.hydrostaticTarget ~= nil and ( 0 >= self.hydrostaticTarget or self.hydrostaticTarget >= 1 ) then
								t = 0
							elseif t <= 0 then
								t = 1+e
							else
	...
	-- automatic shifting			
            ...
            local testRpm
						...
						if self.vehicle.mrGbMS.TargetSpeed >= 0 then
							testRpm = Utils.clamp( math.abs( self.vehicle.mrGbMS.TargetSpeed - 3.6 * spd ) / self.maxForwardSpeed, 0, 1 )
						else
	...
	if self.noTransmission then
		...
		if self.noTorque then
			self.lastThrottle = 0
		elseif self.vehicle.mrGbMS.OnlyHandThrottle then
			self.lastThrottle = handThrottle
		else
  ...
  self.rpmIncFactor   = self.rpmIncFactor + self.vehicle.mrGbML.smoothMedium * ( r - self.rpmIncFactor )
	...
	--if      ( ( self.ptoOn and ( self.hydroEff ~= nil or self.vehicle.mrGbMS.PowerManagement ) )
	--			 or handThrottle >= 0 )
		if      ( self.vehicle.OnlyHandThrottle
					 or ( handThrottle >= 0 
						and self.hydrostaticFactor ~= nil 
					--and self.hydrostaticFactor < self.vehicle.mrGbMS.HydrostaticMax - mrGearboxMogli.eps ) )
						) )
...
function mrGearboxMogliMotor:getLimitedRpm( rpm )
	...
	local maxRpm = math.min( self.maxAllowedRpm, self.lastMotorRpm + self.tickDt * self.rpmIncFactor )
...
function mrGearboxMogliMotor:getClutchPercent( targetRpm, openRpm, closeRpm, fromPercent, toPercent )
	...
	if self.lastMotorRpm < self.stallRpm and self.stallRpm < openRpm then
