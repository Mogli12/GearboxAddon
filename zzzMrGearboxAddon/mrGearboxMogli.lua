...
mrGearboxMogli.ptoRpmFactorEco      = 0.5  -- reduce PTO RPM in eco mode; e.g. 1600 with PTO and 2200 rated RPM
...
--self.mrGbMS.ClutchExponent          = Utils.getNoNil(getXMLFloat(xmlFile, xmlString .. "#clutchExponent"), 0.7 )
	...
	if i > 0 then   
		...
		self.mrGbMS.HydrostaticIncTime  = Utils.getNoNil( getXMLFloat(xmlFile, xmlString .. ".hydrostatic#minMaxTimeMs"), revUpMs * 0.9 )
		self.mrGbMS.HydrostaticStart    = Utils.getNoNil( getXMLFloat(xmlFile, xmlString .. ".hydrostatic#startFactor"), Utils.clamp( 0.6, self.mrGbMS.HydrostaticMin, 1 )  )
		local sc = getXMLBool(xmlFile, xmlString .. ".hydrostatic#startWithClutch")
		if sc == nil then
			self.mrGbMS.HydrostaticLaunch = false --self.mrGbMS.HydrostaticMin * self.mrGbMS.Gears[1].speed < 2 -- m/s
		else
...
function mrGearboxMogli:mbIsSoundActive()
	if self.isClient and self.isMotorStarted and ( self.isEntered or not ( self.steeringEnabled ) ) then
		return true
	end
	return false
end
...
-- inputs	
	...
	if self.isMotorStarted and self.mrGbML.motor ~= nil then
		...
		if self.mrGbMS.GrindingSoundVolume > 0 then
			if mrGearboxMogli.mbIsSoundActive( self ) and self.mrGbMS.GrindingGearsPlay then
				self.mrGbMS.GrindingGearsPlay = false
...
--**********************************************************************************************************			
		...
		if mrGearboxMogli.mbIsSoundActive( self ) and ( self.mrGbMB.soundPitchScale ~= nil or self.mrGbMB.soundRunPitchScale ~= nil ) then			
			...
			if self.mrGbMG.modifyVolume and self.internalSound == nil then	
				local motorLoad 
				if self.isServer then 
				...
				if self.sampleMotor.sample ~= nil then
					...
					if self.mrGbMB.soundIndoorIndex ~= nil then
						self.indoorSounds.sounds[self.mrGbMB.soundIndoorIndex].outdoor = self.sampleMotor.volume
						self.indoorSounds.sounds[self.mrGbMB.soundIndoorIndex].indoor  = self.sampleMotor.volume * self.indoorSounds.sounds[self.mrGbMB.soundIndoorIndex].indoorFactor
					end
				end
				if self.sampleMotorRun.sample ~= nil then
					local f0 = Utils.clamp( math.abs( self.lastRoundPerMinute ) / math.max( 1, self.mrGbMS.OrigMaxRpm - self.mrGbMS.OrigMinRpm ), 0, 1 )
					...
					if self.mrGbMB.soundRunIndoorIndex ~= nil then
						self.indoorSounds.sounds[self.mrGbMB.soundRunIndoorIndex].outdoor = self.sampleMotor.volume
						self.indoorSounds.sounds[self.mrGbMB.soundRunIndoorIndex].indoor  = self.sampleMotor.volume * self.indoorSounds.sounds[self.mrGbMB.soundRunIndoorIndex].indoorFactor
					end
				end
...
function mrGearboxMogli:mrGbMGetAutoClutch()
	return self.mrGbMS.AllAuto or self.mrGbMS.AutoClutch or self.mrGbMS.Hydrostatic
end
...
--**********************************************************************************************************	
	...
	if self.vehicle.mrGbMS.IsCombine then
		if self.vehicle:getIsTurnedOn() then
			...
			elseif self.lastMissingTorque > 0 then
				...
				targetRpmC        = Utils.clamp( self.rpmPowerCurve:get( math.max( requestedTorque, 1.25 * self.lastPtoTorque ) * math.max( self.prevNonClampedMotorRpm, self.idleRpm ) ), self.vehicle.mrGbMS.ThreshingMinRpm, self.vehicle.mrGbMS.ThreshingMaxRpm )
			end			
	...
	local pp = math.min( ( self.lastPtoTorque + self.lastMissingTorque ) * math.max( self.prevNonClampedMotorRpm, self.idleRpm ), self.maxPower )
	local rp = pp + acc * ( self.maxPower - pp )
	
	if rp > currentPower then
...
--**********************************************************************************************************		
	if     self.vehicle.mrGbMS.NeutralActive
			...
			or ( self.lastMotorRpmS                   < math.max( minRpmReduced, 0.5*( self.stallRpm + self.idleRpm ) )
			 ...
			 and self.vehicle:mrGbMGetAutoClutch() ) then
	-- neutral or braking
...
--		end
	...
	-- automatic shifting				
			...
			elseif self.vehicle:mrGbMGetAutomatic() then		
				...
				if     g_currentMission.time > self.vehicle.mrGbML.autoShiftDownTimer 
					...
					for i = 1,maxGear do
						...
						if isValidEntry then
							...
							elseif self.hydroEff ~= nil then
								--test = rpm
								local test = self.hydroEff:get( Utils.clamp( rpm / self.targetRpm, self.vehicle.mrGbMS.HydrostaticMin, self.vehicle.mrGbMS.HydrostaticMax ) )
							...
							--end
								if i ~= currentGear then
									testLag = 25 * math.floor( 0.5 + 0.04 * timeToShift )
								end
								...
								if i ~= currentGear and power < currentPower then
									...
									testPwr = math.max( self.requestedPower - power, 0 )
								end
								
								testLag = Utils.clamp( testLag * 0.0005, 0, 1 )
								testRpm = Utils.clamp( 2 * testRpm / ( self.ratedRpm - self.idleRpm ), 0, 1 ) 
								testPwr = Utils.clamp( testPwr / self.maxPower, 0, 1 )
								
								local test = 0
								if i ~= currentGear then test = 0.01   end
								if test < testLag then test = testLag end
								if test < testRpm then test = testRpm end
								if test < testPwr then test = testPwr end
								
							--print(string.format("G: %d R: %d || L: %0.3f R: %0.3f P: %0.3f => %0.3f (%0.3f)", i2g, i2r, testLag, testRpm, testPwr, test, Utils.getNoNil( scoreRpm, -1 ) ) )
				...
				--	print(string.format("G: %d R: %d || new G: %d R: %d", self.vehicle.mrGbMS.CurrentGear, self.vehicle.mrGbMS.CurrentRange, i2g, i2r ) )
					end
	...
	--elseif self.hydroEff ~= nil then
		if self.hydroEff ~= nil then
			...
			if self.ptoOn and self.lastMissingTorque <= 0 then
				local t0 = math.max( minRpmReduced, t ) - Utils.clamp( self.lastLimitRpm - self.nonClampedMotorRpm, 0, mrGearboxMogli.hydroPtoDiff )
		...
		--if     self.vehicle.mrGbMS.CurrentGear == 1 then
			...
			elseif self.nonClampedMotorRpm <= self.stallRpm 
					or self.ptoOn                 then
				self.hydrostaticFactor = self.hydrostaticFactor + mrGearboxMogli.smoothHydro * ( h - self.hydrostaticFactor )
			else
...
--elseif self.hydroEff == nil then
	...
	if      self.lastMissingTorque  > 0 
		...
		self.torqueRpmReduction   = math.min( 500, self.torqueRpmReduction + math.min( 0.2, self.lastMissingTorque / self.lastMotorTorque ) * self.tickDt * self.vehicle.mrGbMS.RpmDecFactor )
	--local rpm                 = self.nonClampedMotorRpm - self.stallRpm
	--self.torqueRpmReduction   = math.min( math.min( rpm, 500 ), self.torqueRpmReduction +
	--                                      math.min( math.max( 0, rpm * self.lastMissingTorque / self.lastMotorTorque - self.torqueRpmReduction ), self.tickDt * self.vehicle.mrGbMS.RpmDecFactor ) )
...
function mrGearboxMogliMotor:getClutchPercent( currentRpm, targetRpm, openRpm, closeRpm, accelerationPedal )
	...
	--if currentRpm > openRpm and minPercent < self.autoClutchPercent then
