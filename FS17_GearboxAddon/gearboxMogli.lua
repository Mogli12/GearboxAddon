...
gearboxMogliGlobals.DefaultRevUpMs1       = 15000 -- ms time between idle and rated RPM with full load
...
gearboxMogliGlobals.smoothGearRatioTC     = true  -- smooth gear ratio with torque converter
...
-- state variables with setter methods	
	...
	gearboxMogli.registerState( self, "ManualDiffLock", 1  )
...
--**************************************************************************************************			
	...
	local hit = 1000
	local hdt = 1000
	self.mrGbMS.HydrostaticThrottle       = getXMLBool(xmlFile, xmlString .. ".hydrostatic#throttleControlled")
	...
	if self.mrGbMS.HydrostaticEfficiency == nil then
		...
		--self.mrGbMS.HydrostaticCoupling = "Direct"
			...
			table.insert(self.mrGbMS.HydrostaticEfficiency, {time=0.85 , v=0.95})
			table.insert(self.mrGbMS.HydrostaticEfficiency, {time=1 , v=0.9})			
		elseif self.mrGbMS.HydrostaticProfile == "Variator" then
		
			local range = Utils.getNoNil( getXMLFloat(xmlFile, xmlString .. ".hydrostatic#range"), 4 )
			local delta = ( range - 1 ) / ( range + 1 )
			self.mrGbMS.HydrostaticMin = 1 - delta 
			self.mrGbMS.HydrostaticMax = 1 + delta 
				
			self.mrGbMS.TransmissionEfficiency = 0.98
			self.mrGbMS.HydrostaticPtoDiff     = 200				
			self.mrGbMS.HydrostaticEfficiency  = {}
			
			if      self.mrGbMS.HydrostaticThrottle == nil 
					and self.mrGbMS.OnlyHandThrottle then
				self.mrGbMS.HydrostaticThrottle = true 
			end 
			
			if hydroCorrectGearSpeed == nil then 
				hydroCorrectGearSpeed = true
			end 
			...
			mrfg = self.mrGbMG.hydroMaxTorqueDirect
			 ...
			 table.insert(self.mrGbMS.HydrostaticEfficiency, {time=0 ,    v=0.1})
			 table.insert(self.mrGbMS.HydrostaticEfficiency, {time=0.05 , v=0.4})
			 table.insert(self.mrGbMS.HydrostaticEfficiency, {time=0.2 ,  v=0.6})
			 table.insert(self.mrGbMS.HydrostaticEfficiency, {time=0.4 ,  v=0.75})
			 table.insert(self.mrGbMS.HydrostaticEfficiency, {time=0.6 ,  v=0.87})
			 table.insert(self.mrGbMS.HydrostaticEfficiency, {time=0.8 ,  v=0.95})
			 ...
			 table.insert(self.mrGbMS.HydrostaticEfficiency, {time=1.4 ,  v=0.87})
			 table.insert(self.mrGbMS.HydrostaticEfficiency, {time=1.6 ,  v=0.75})
			 table.insert(self.mrGbMS.HydrostaticEfficiency, {time=1.8 ,  v=0.6})
			 table.insert(self.mrGbMS.HydrostaticEfficiency, {time=1.95 , v=0.4})
			 table.insert(self.mrGbMS.HydrostaticEfficiency, {time=2 ,    v=0.1})
	...
	if     self.mrGbMS.HydrostaticEfficiency  ~= nil
		...
		if     self.mrGbMS.HydrostaticPressure    ~= nil 
			...
			local tmp
			local i = table.getn( self.mrGbMS.HydrostaticEfficiency )
			...
			elseif tmp ~= nil then 
				print('FS17_GearboxAddon: Error: <hydrostatic minRatio="'..tostring(tmp)..'"> is out of range '..tostring(self.mrGbMS.HydrostaticEfficiency[1].time)
																																																	 '..'..tostring(self.mrGbMS.HydrostaticEfficiency[i].time))
			end
			...
			elseif tmp ~= nil then 
				print('FS17_GearboxAddon: Error: <hydrostatic maxRatio="'..tostring(tmp)..'"> is out of range '..tostring(self.mrGbMS.HydrostaticEfficiency[1].time)
																																																	 '..'..tostring(self.mrGbMS.HydrostaticEfficiency[i].time))
			end	
		end	
...
--**********************************************************************************************************			
	...
	if self.mrGbMS.ModifyDifferentials > 0 and self.isServer then
                ...
                local angle = Utils.convertToDensityMapAngle(Utils.getYRotationFromDirection(dx, dz), g_currentMission.terrainDetailAngleMaxValue);
								...
								if wheel.additionalWheels ~= nil then 
									for _,additionalWheel in pairs(wheel.additionalWheels) do
										...
										width = 0.3 * additionalWheel.width
				...
				for row,info in pairs( infos ) do
					...
					elseif info == "fixed" then					
						if col == 1 then
							...
							local kmh    = minSp + ( maxSp - minSp ) * self.mrGbMS.FixedRatio
...
function gearboxMogli:getSaveAttributesAndNodes(nodeIdent)
	...
	if self.mrGbMS ~= nil then
		...
		if self.mrGbMS.ModifyDifferentials > 0 and self.mrGbMS.ManualDiffLock ~= 1 then
			attributes = attributes.." mrGbMManualDiff=\"" .. tostring( self.mrGbMS.ManualDiffLock ) .. "\""     
		end 
	end 
...
function gearboxMogli:loadFromAttributesAndNodes(xmlFile, key, resetVehicles)
	...
	if self.mrGbMS ~= nil then
		...
		b = getXMLBool( xmlFile, key .. "#mrGbMManualDiffLock" )
		if b == nil then 
		elseif b then 
			self.mrGbMS.ManualDiffLock = 1
		else
			self.mrGbMS.ManualDiffLock = 0
		end
		...
		gearboxMogli.loadHelperInt( self, xmlFile, key .. "#mrGbMManualDiff"    , "ManualDiffLock"    )
...
function gearboxMogli:checkGearShiftDC( new, what, noEventSend )
	...
	if      gearboxMogli.mrGbMCheckDoubleClutch( self, dc, noEventSend )
		...
		return gearboxMogli.checkTransmissionInOutRpm( self, w, noEventSend )
	end
...
--**********************************************************************************************************	
-- gearboxMogli:checkTransmissionInOutRpm
--**********************************************************************************************************	
function gearboxMogli:checkTransmissionInOutRpm( outRpm, noEventSend )

	if not self.isServer or self.motor.transmissionInputRpm == nil then 
		return true 
	end

	local v = 0			
	if self.motor.transmissionInputRpm < outRpm then
		v = ( outRpm - self.motor.transmissionInputRpm ) / self.mrGbMG.grindingMinRpmDelta
	else
		v = ( self.motor.transmissionInputRpm - outRpm ) / self.mrGbMG.grindingMaxRpmSound
		if v > 1 and self.motor.transmissionInputRpm - outRpm < self.mrGbMG.grindingMaxRpmDelta then
			v = 0.999
		end
	end
	...
	if     v > 1 then
		...
		self:mrGbMSetState( "WarningText", string.format( "%s (in: %4.0f / out: %4.0f)",
																											...
																											self.motor.transmissionInputRpm, outRpm ))
		self.mrGbMS.GrindingGearsVol = 0
...
function gearboxMogli:newUpdateWheelsPhysics( superFunc, dt, currentSpeed, acc, doHandbrake, requiredDriveMode, ... )
	...
	--	-- full power to the "bauer"
		elseif  self.mrGbMS.Hydrostatic 
				and self.mrGbMS.ConstantRpm
				and not ( self.mrGbMS.HydrostaticThrottle )
				and self.mrGbMS.FixedRatio <= gearboxMogli.eps then
...
function gearboxMogli.updateDestroyCommonArea(startWorldX, startWorldZ, widthWorldX, widthWorldZ, heightWorldX, heightWorldZ, limitToField, destroyFruits)
	...
	for i=1, table.getn(g_currentMission.dynamicFoliageLayers) do
		...
		local x, z, widthX, widthZ, heightX, heightZ = Utils.getXZWidthAndHeight(nil, startWorldX, startWorldZ, widthWorldX, widthWorldZ, heightWorldX, heightWorldZ);
		setDensityMaskedParallelogram(id, x, z, widthX, widthZ, heightX, heightZ, 0, numChannels, g_currentMission.terrainDetailId, g_currentMission.terrainDetailTypeFirstChannel, g_currentMission.terrainDetailTypeNumChannels, 0);
	end
end
...
--if self.mrGbMS.Hydrostatic and not ( self.mrGbMS.ConstantRpm ) then
	if self.mrGbMS.Hydrostatic then
		local _,minSp,maxSp = self:mrGbMGetGearSpeed()
		local refRpm = self.mrGbMS.RatedRpm
		local limit  = gearboxMogli.huge
		...
		self.mrGbMUI.FixedRatioSteps = math.max( 1, ( maxSp - minSp ) / self.mrGbMG.uiFixedRatioStep )
		...
		for i=1,math.floor( self.mrGbMUI.FixedRatioSteps ) do
			local kmh = minSp + ( maxSp - minSp ) * i / self.mrGbMUI.FixedRatioSteps
			if kmh <= limit then
	...
	if self.mrGbMS.ModifyDifferentials <= 0 then 
		self.mrGbMS.ManualDiffLock = 0
		self.mrGbMUI.ManualDiffLock = { gearboxMogli.getText( "gearboxMogliTEXT_DISABLED", "off" ) }
	else
		self.mrGbMUI.ManualDiffLock = { gearboxMogli.getText( "gearboxMogliTEXT_DISABLED", "off" ), 
																		gearboxMogli.getText( "gearboxMogliTEXT_ENABLED", "on" ), 
																		gearboxMogli.getText( "gearboxMogliTEXT_AI", "hired worker" ) }
	end
...
function gearboxMogli:mrGbMUISetDiffLockMiddle( value )
	if      self.mrGbMS.ModifyDifferentials > 0
			and self.mrGbMS.ManualDiffLock      > 0
			and self.mrGbMS.IsOn 
			and ( self.steeringEnabled or self.mrGbMS.ManualDiffLock == 2 ) then
