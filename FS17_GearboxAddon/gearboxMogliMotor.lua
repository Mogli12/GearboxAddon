...
--**********************************************************************************************************	
	...
	if self.vehicle.mrGbMS.ConstantRpm then
		if     self.vehicle.mrGbMS.IsCombine   then
			self.targetRpm = self.minRequiredRpm
		elseif self.vehicle.mrGbMS.Hydrostatic then
	...
	if self.timeShiftTab == nil then
		...
		if not ptoToolOn then
			self.timeShiftTab = { { t=g_currentMission.time, v=targetRpm, s=self.targetRpmS, p=requestedPower, q=self.requestedPowerS } }
		else
			self.timeShiftTab = {}
		end
	...
	if      self.lastMotorTorque    > gearboxMogli.eps
			...
			and utt                     > 0.8 * self.lastMotorTorque then
		...
		elseif self.noTransmission then
			...
			reduceIt = 2
		elseif  self.vehicle.mrGbMS.IsCombine then
			reduceIt = -2
	...
	if self.torqueRpmReduction ~= nil then
		self.targetRpm = math.min( self.targetRpm, self.torqueRpmReference - self.torqueRpmReduction )
...
--**********************************************************************************************************		
		...
		-- initialize 
			minThrottle   = math.max( 0.2, self.vehicle.mrGbMS.HandThrottle )
		else
			if     self.nonClampedMotorRpm <= minRpmReduced then
				minThrottle = 1
			elseif self.nonClampedMotorRpm < self.minRequiredRpm then
				minThrottle = ( self.minRequiredRpm - self.nonClampedMotorRpm ) / ( self.minRequiredRpm - minRpmReduced )
			else
				minThrottle = 0
			end	
		end
	...
	-- hydrostatic drive
			if self.vehicle.mrGbMS.Hydrostatic then
				...
				if self.vehicle.mrGbMS.FixedRatio > gearboxMogli.eps then
					spdFix = self.vehicle.mrGbMS.FixedRatio * hMax * self.vehicle.mrGbMS.AutoMaxGearSpeed
				end
				...
				if self.vehicle.mrGbML.gearShiftingNeeded == 0 then
					...
					local hFix = -1
					if spdFix > 0 then
						hFix  = Utils.clamp( spdFix / self.vehicle.mrGbMS.CurrentGearSpeed, hMin, hMax )
						n0    = minRpmReduced
					end
					...
					if ( getMaxPower or self.torqueRpmReduction ~= nil ) and currentSpeedLimit > self.vehicle.lastSpeedReal*1000 then
						currentSpeedLimit = self.vehicle.lastSpeedReal*1000
					end
					...
					elseif not ( self.vehicle.mrGbMS.HydrostaticDirect ) then
						...
						for f=0,ti do
							...
							if mt > gearboxMogli.eps and mt > self.ptoMotorTorque then
								...
								local rt = accelerationPedal * ( mt - self.ptoMotorTorque ) 
					...
					local hFx = 1
					if hTgt > gearboxMogli.eps and self.vehicle.mrGbMS.HydrostaticLossFxRpmRatio > 0 and self.torqueRpmReduction == nil then
						local f 
					...
					if spdFix > 0 and accelerationPedal >= -gearboxMogli.accDeadZone then
					-- fixed ratio and not braking
						local s      = self.vehicle.lastSpeedReal*1000
						local spdMin = spdFix * minRpmReduced       / self.vehicle.mrGbMS.RatedRpm
						local spdMax = spdFix * self.minRequiredRpm / self.vehicle.mrGbMS.RatedRpm
						if     s >= spdMax then
							minThrottle = 0
						elseif s <= spdMin then
							minThrottle = 1
						else
							minThrottle = ( spdMax - s ) / ( spdMax - spdMin )
						end
					end
			...
			elseif self.vehicle:mrGbMGetAutomatic() 
				...
				for i,p in pairs( possibleCombinations ) do
					...
					if      isValidEntry 
						...
						local refTime = self.lastClutchClosedTime
						if      p.gearSpeed > self.vehicle.mrGbMS.CurrentGearSpeed + gearboxMogli.eps
								and refTime     < self.lastAccelerationStart then
							refTime = self.lastAccelerationStart
						end
	...
	-- min thorttle part II		
		...
		self.minThrottle  = Utils.clamp( self.minThrottle + Utils.clamp( minThrottle - self.minThrottle, -maxDeltaThrottle, maxDeltaThrottle ), 0, 1 )
	...
	if self.noTransmission then
		if self.vehicle.mrGbML.afterShiftClutch ~= nil then
